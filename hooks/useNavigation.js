...
export const useNavigation = () => {
  ...
  useEffect(() => {
    ...
    unsubscribeRef.current = navigationService.subscribe(
    ...
    async (origin, destination, options = {}) => {
      try {
        ...
        if (options.optimize) {
          ...
          const routes = await navigationService.optimizeRoute(
            origin,
            destination,
            options
          );
          ...
          selectedRoute = await navigationService.startNavigation(
            origin,
            destination
          );
        }
  ...
  const getRouteAlternatives = useCallback(
    async (origin, destination, options = {}) => {
      try {
        const routes = await navigationService.optimizeRoute(
          origin,
